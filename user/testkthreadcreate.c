...
int main(int argc, char** argv){
    ...
    int tid = kthread_create(func,memory,MAX_STACK_SIZE);
    ...
    sleep(5);
    if((tid = kthread_create(errorfunc,0,MAX_STACK_SIZE)) != -1){
        kthread_join(tid,0);
    ...
    if((tid = kthread_create(errorfunc,memory,2*MAX_STACK_SIZE)) != -1){
        kthread_join(tid,0);
    ...
    for(int i=0;i<9;i++){
        ...
        if((ktid[i] = kthread_create(fib,memories[i],MAX_STACK_SIZE)) == -1){
            fprintf(2,"[ERROR] TEST 4 FAILED\n got -1 in iteration:%d\n",i);
            return 0x66;
        }
    }
    memories[10]= malloc(MAX_STACK_SIZE/2);
    if((ktid[10] = kthread_create(fib,memories[10],MAX_STACK_SIZE)) != -1){
        fprintf(2,"[ERROR] TEST 4 FAILED\n got tid = %d instead of -1.\n",ktid[10]);
        return 0x66;
    }
    for(int i=0;i<11;i++){
